...
def jsonl_to_json(jsonl_dir: str):
    ...
    with open(jsonl_dir, "r") as f:
        for line in f:
            ...
            for qcode, description in data_dict.items():
                data_dict[qcode] = description
...
def main():
    ...
    parser.add_argument("--input_dir", type=str, required=True, help="path to preprocessed dump of descriptions")
    parser.add_argument("--output_dir", type=str, required=True, help="path to output directory")
    parser.add_argument("--num_files", type=int, default=10000)
    args = parser.parse_args()

    input_dir = join(args.input_dir, "descriptions")

    output_dir = join(args.output_dir, "descriptions")
    ...
    for input_file in tqdm(input_files):
        with open(input_file) as f:
            for line in f:
                data_dict = json.loads(line)
                qcode, description = data_dict["qid"], data_dict["description"]
